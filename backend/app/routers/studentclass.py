...
async def read_specific_studentclass(
...
async def read_all_studentclasses(db: Session = Depends(get_session)):
    return get_studentclasses(db=db)


@router.post("/")
async def write_studentclass(
    studentclass: StudentClass, db: Session = Depends(get_session)
):
